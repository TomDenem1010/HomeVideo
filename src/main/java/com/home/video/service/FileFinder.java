...
public class FileFinder {
  ...
  public List<VideoDto> findVideosByPath(final String path) {
    ...
    try (Stream<Path> paths = Files.walk(Paths.get(path))) {
      paths
          ...
          .forEach(
              actualPath -> {
                Map<String, Object> splittedName =
                    videoNameSplitter.split(actualPath.getFileName().toString());
                response.add(
                    new VideoDto(
                        null,
                        ...
                        getSize(actualPath),
                        getActorDtos(
                            (List<String>) splittedName.get(VideoNameSplitter.ACTORS_KEY), now),
                        Status.ACTIVE,
                        now,
                        now));
              });
  ...
  private FolderDto getFolderDto(final Path path, final LocalDateTime now) {
    return new FolderDto(null, getParentName(path), Status.ACTIVE, now, now);
  }
