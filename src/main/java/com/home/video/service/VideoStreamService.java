...
public class VideoStreamService {
  ...
  public VideoStreamService(
      final FileFinder fileFinder, final @Value("${video.path}") String basePath) {
    this.fileFinder = fileFinder;
    this.basePath = basePath;
  }

  public ResponseEntity<ResourceRegion> streamVideo(
      final String rangeHeader, final HttpServletRequest request) {
    ...
    List<HttpRange> httpRanges =
        rangeHeader != null ? HttpRange.parseRanges(rangeHeader) : List.of();
    ...
    return ResponseEntity.status(HttpStatus.PARTIAL_CONTENT)
        .contentType(
            MediaTypeFactory.getMediaType(videoResource).orElse(MediaType.APPLICATION_OCTET_STREAM))
        .body(region);
  }
        ...
        URLDecoder.decode(
                URLDecoder.decode(
                    request
                        .getRequestURI()
                        .substring(
                            request.getRequestURI().indexOf("/stream/") + "/stream/".length()),
